...
namespace com.wer.sc.data.receiver
{
    ...
    public class DataReceiver
    {
        ...
        private List<market.InstrumentInfo> instruments = new List<market.InstrumentInfo>();
        ...
        private void OnReturnInstrument(object sender, ref List<market.InstrumentInfo> instruments)
        {
        ...
        private void WriteInstruments(List<market.InstrumentInfo> instruments)
        {
            instruments.Sort(delegate (market.InstrumentInfo a, market.InstrumentInfo b) { return a.Symbol.CompareTo(b.Symbol); });
            SaveInstruments(instruments);
        }

        private void SaveInstruments(List<market.InstrumentInfo> instruments)
        {
            JsonUtils_Instrument.Save(marketPath + "Instruments_" + this.tradingDay + ".json", instruments);
        }

        private List<market.InstrumentInfo> LoadInstruments()
        {
        ...
        private string[] GetCodes(List<market.InstrumentInfo> instruments)
        {
            int subCount = instruments.Count;
            string[] codes = new string[subCount];
            for (int i = 0; i < subCount; i++)
            {
                market.InstrumentInfo instrument = instruments[i];
