...
namespace com.wer.sc.plugin.cnfutures.config
{
    ...
    /// 股票或期货信息的原始数据加载
    /// </summary>
    internal class DataLoader_InstrumentInfo
    {
        ...
        private void initCatelogs()
        {
            ...
            for (int i = 0; i < lines.Length; i++)
            {
                ...
                dic_CatelogId_CatelogName.Add(strs[0].ToUpper(), strs[1]);
                catelogs.Add(strs[0]);
            }
        }
        ...
        {
            return GetMarketByVariety(dicInstruments[code.ToUpper()].Catelog);
        }

        public string GetMarketByVariety(string variety)
        {
            return dic_Catelog_Exchange[variety.ToUpper()];
        }
        ...
        public String GetVarietyNameByCode(String code)
        {
        ...
        {
            return dic_CatelogId_CatelogName[variety.ToUpper()];
        }
